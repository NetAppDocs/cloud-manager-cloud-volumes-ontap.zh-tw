...
* link:task-deploying-otc-aws.html["在 Cloud Volumes ONTAP AWS 中啟動"]
...
* link:task-deploying-otc-azure.html["在 Cloud Volumes ONTAP Azure 中啟動"]
...
* link:task-deploying-gcp.html["在Cloud Volumes ONTAP Google Cloud上啟動"]
...
* 在AWS中、年度合約可從AWS Marketplace的公開優惠中取得。部署Cloud Volumes ONTAP 執行功能的環境時、Cloud Manager會提示您從「詳細資料與認證」頁面訂閱。
...
. link:task-manage-capacity-licenses.html["將授權新增至Cloud Manager"]
