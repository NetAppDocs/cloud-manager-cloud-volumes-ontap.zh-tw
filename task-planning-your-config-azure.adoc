...
--
Azure 磁碟大小:: 啟動 Cloud Volumes ONTAP 時、您必須選擇集合體的預設磁碟大小。Cloud Manager 會將此磁碟大小用於初始 Aggregate 、以及使用簡易資源配置選項時所建立的任何其他 Aggregate 。您可以建立使用不同於預設磁碟大小的 Aggregate link:task-create-aggregates.html["使用進階配置選項"]。
+
--
...
Azure 中的一個支援本地 NVMe 儲存設備的組態、可將其用作 _Flash Cache 以獲得更好的效能。 Cloud Volumes ONTAP Cloud Volumes ONTAP link:concept-flash-cache.html["深入瞭解 Flash Cache"]。
