...
summary: 帳戶管理員可在Cloud Manager中啟用一項設定、以避免Cloud Volumes ONTAP 在Azure維護活動期間發生有關還原儲存設備的問題。啟用此設定時 Cloud Volumes ONTAP 、不支援 CIFS 會鎖定並重設作用中的 CIFS 工作階段。 
---
...
[role="lead"]
帳戶管理員可在Cloud Manager中啟用一項設定、以避免Cloud Volumes ONTAP 在Azure維護活動期間發生有關還原儲存設備的問題。啟用此設定時 Cloud Volumes ONTAP 、不支援 CIFS 會鎖定並重設作用中的 CIFS 工作階段。
...
您必須先建立連接器、才能變更 Cloud Manager 設定。 https://docs.netapp.com/us-en/cloud-manager-setup-admin/concept-connectors.html#how-to-create-a-connector["瞭解方法"^]。
