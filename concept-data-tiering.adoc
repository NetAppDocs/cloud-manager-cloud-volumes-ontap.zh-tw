...
Cloud Manager建立S3儲存區時、會使用下列預設設定：
...
+
--
Cloud Manager會為Cloud Volumes ONTAP 每個運作環境建立一個新的儲存帳戶、其中包含一個容器。儲存帳戶名稱為隨機。並不會針對每個 Volume 建立不同的容器。

Cloud Manager會以下列設定建立儲存帳戶：
...
+
--
Cloud Manager會為每個工作環境建立一個儲存區、並將其命名為「網路資源池」、「叢集唯一識別碼」。並不會針對每個 Volume 建立不同的儲存區。

Cloud Manager建立儲存庫時、會使用下列預設設定：
