...
** 與 Connector 相關的服務帳戶 link:https://occm-sample-policies.s3.amazonaws.com/Policy_for_Cloud_Manager_3.9.10_GCP.yaml["應具備最新權限"^]。
...
** link:task-deploying-gcp.html#licensing["瞭解精靈中您想要使用的授權方法所需的內容"]。
...
+
image:screenshot_cvo_licensing_gcp.gif["「授權」頁面的快照"]
...
NOTE: 如果所選版本有較新的發行候選版本、一般可用度或修補程式版本、 Cloud Manager 會在建立工作環境時、將系統更新至該版本。例如、如果您選擇 Cloud Volumes ONTAP 了「更新」功能、就會發生更新。更新不會從一個版本發生到另一個版本、例如從 9.7 到 9.8 。
